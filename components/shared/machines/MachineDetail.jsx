...
import { getMachineImage, getMachineTag, generateQR, getMachineCertificate,  } from '../../../api/machine';
...
export default function MachineDetail() {
  ...
  const [updateCertificate, setUpdateCertificate] = useState(false);
  ...
  const downloadCertificate = (extension) => {
    getMachineCertificate(currentMachine._id)
              .then(response => {
                  fileDownload(response, `${currentMachine.nomMaquina}-certificado.${extension}`); 
              })
  }

  const updateMachineFunction = () => {
    const currentMachineData = {
      ...currentMachineState,
      certificado: certificateAttachment
    }

    dispatch(updateMachineByIdAction(currentMachineData))
  }
  return (
    <div className='mx-5 mt-3'>
        ...
        <Card className='card shadow'>
          <div className='d-flex flex-column flex-md-row justify-content-center'>
            <div className='pointer' onClick={() => setShowUpdateImageMachine(true)}>
              <MachineImage alt={currentMachine.nomMaquina} getImage={getMachineImage} param={currentMachine?._id} status={currentMachine?.foto_equipo ? true : false} width={'500px'} height={'300px'} />
            </div>
            ...
            <div className='ms-5 pointer' onClick={() => setShowUpdateTagImageMachine(true)}>
              <MachineTagImage alt={currentMachine.nomMaquina} getImage={getMachineTag} param={currentMachine?._id} status={currentMachine?.foto_etiqueta_calibracion ? true : false} width={'500px'} height={'300px'} />
            </div>
          ...
          <ListGroup onKeyPress={handleKeyPress} className="list-group-flush">
            ...
            <ListGroup.Item >
                    <div className='d-flex flex-between align-item-center w-100 me-5'>
                      <div>
                        {currentMachine.nomMaquina}
                      </div>
                      <Button onClick={() => {downloadCertificate(currentMachine.certificado.fileType)}}>Descargar</Button>
                    </div>
                  </ListGroup.Item>
          </ListGroup>
        </Card>
      ...
      <UpdateCertificate updateCertificate={updateCertificate} setUpdateCertificate={setUpdateCertificate} setCertificateAttachment={setCertificateAttachment} updateMachineFunction={updateMachineFunction}/>
      <SaveButton updateCurrentMachine={updateCurrentMachine} />
    </div>
  )
}

function UpdateCertificate({updateCertificate, setUpdateCertificate, setCertificateAttachment, updateMachineFunction}){
  return (
    <div>
     {
      !updateCertificate 
      ? (
        <div className='d-flex flex-between mt-3'>
            <button onClick={() => setUpdateCertificate(true)} className='btn btn-action-primary w-50 ms-auto me-auto'>Actualizar Certificado</button>
        </div>
      )
      : (
        <div className='d-flex flex-column w-100'>
          <label className="form-label">Subir Certificado</label>
          <input type='file' className='form-control' onChange={(e)=>setCertificateAttachment(e?.target.files[0])} />
          <button onClick={() =>{updateMachineFunction(); setUpdateCertificate(false)}} className='btn btn-action-primary w-50 ms-auto me-auto mt-3'>Actualizar Certificado</button>
        </div>
      )
     }
    </div>
